...
class CustomModel(tf.keras.Model):
	...
	def __init__(self):
		...
		self.pooling1 = tf.keras.layers.MaxPooling2D(poolsize[0],strides=None,padding='valid',data_format=None)
		
		self.convolution2 = tf.keras.layers.Conv2D(features_values[1],Filter_size[1], activation = 'relu', name = 'convolution2')
		self.pooling2 = tf.keras.layers.MaxPooling2D(poolsize[1],strides=None,padding='valid',data_format=None)
		
		self.convolution3 = tf.keras.layers.Conv2D(features_values[2],Filter_size[2], activation = 'relu', name = 'convolution3')
		self.pooling3 = tf.keras.layers.MaxPooling2D(poolsize[2],strides=None,padding='valid',data_format=None)
		
		self.convolution4 = tf.keras.layers.Conv2D(features_values[3],Filter_size[3], activation = 'relu', name = 'convolution4')
		self.pooling4 = tf.keras.layers.MaxPooling2D(poolsize[3],strides=None,padding='valid',data_format=None)
		
		self.convolution5 = tf.keras.layers.Conv2D(features_values[4],Filter_size[4], activation = 'relu', name = 'convolution5')
		self.pooling5 = tf.keras.layers.MaxPooling2D(poolsize[4],strides=None,padding='valid',data_format=None)
		...
		rnn_layer2 = tf.keras.layers.LSTM(lstm_numbers[1],return_sequences = True, stateful = True, go_backwards = True, name = 'rnn_layer2')
		
		#Build Bidirectional layer
		self.bidirectional_layer = tf.keras.layers.Bidirectional(rnn_layer1, backward_layer = rnn_layer2)
